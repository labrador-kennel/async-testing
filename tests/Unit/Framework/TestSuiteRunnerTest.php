...
use Labrador\AsyncUnit\Test\Unit\Framework\Stub\FailingMockBridgeStub;
...
class TestSuiteRunnerTest extends PHPUnitTestCase {
    ...
    public function setUp() : void {
        ...
        $this->emitter->register(Events::TEST_PROCESSED, $this->listener);
    }
    ...
    public function testTestPassedEventsEmittedAfterTestProcessedEvent() {
        ...
        foreach ([Events::TEST_PROCESSED, Events::TEST_PASSED] as $event) {
            $this->emitter->register($event, $listener);
        }
        $this->parseAndRun($this->implicitDefaultTestSuitePath('SingleTest'));
    ...
    public function testTestFailedEventsEmittedAfterTestProcessedEvent() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_PROCESSED, Events::TEST_FAILED] as $event) {
            $this->emitter->register($event, $listener);
        }
    ...
    public function testTestErrorEventEmittedAfterTestProcessedEvent() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_PROCESSED, Events::TEST_ERRORED] as $event) {
            $this->emitter->register($event, $listener);
        }
    ...
    public function testTestDisabledEventsEmittedAfterTestProcessedEvent() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_PROCESSED, Events::TEST_DISABLED] as $event) {
            $this->emitter->register($event, $listener);
        }
    ...
    public function testTestSuiteStartedAndFinishedEventsEmittedInOrder() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_SUITE_STARTED, Events::TEST_PROCESSED, Events::TEST_SUITE_FINISHED] as $event) {
            $this->emitter->register($event, $listener);
        }
        $this->parseAndRun($this->implicitDefaultTestSuitePath('SingleTest'));
    ...
    public function testTestCaseProcessingEventEmitted() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_CASE_STARTED, Events::TEST_PROCESSED, Events::TEST_CASE_FINISHED] as $event) {
            $this->emitter->register($event, $listener);
        }
        $this->parseAndRun($this->implicitDefaultTestSuitePath('SingleTest'));
    ...
    public function testTestProcessingEventsEmittedInOrder() {
        $listener = $this->createRecordingListener();
        foreach ([Events::TEST_PROCESSED, Events::PROCESSING_FINISHED, Events::PROCESSING_STARTED] as $event) {
            $this->emitter->register($event, $listener);
        }
        $this->parseAndRun($this->implicitDefaultTestSuitePath('SingleTest'));
